...
type SimulatedBackend struct {
	database   entrustdb.Database   // In memory database to store our testing data
	blockchain *core.BlockChain // Trustmachine blockchain to handle the consensus
...
func NewSimulatedBackend(alloc core.GenesisAlloc) *SimulatedBackend {
	database, _ := entrustdb.NewMemDatabase()
	genesis := core.Genesis{Config: params.AllProtocolChanges, Alloc: alloc}
	genesis.MustCommit(database)
	blockchain, _ := core.NewBlockChain(database, genesis.Config, entrustash.NewFaker(), new(event.TypeMux), vm.Config{})
...
func (b *SimulatedBackend) CallContract(ctx context.Context, call trustmachine.CallMsg, blockNumber *big.Int) ([]byte, error) {
	b.mu.Lock()
	defer b.mu.Unlock()
...
func (b *SimulatedBackend) PendingCallContract(ctx context.Context, call trustmachine.CallMsg) ([]byte, error) {
	b.mu.Lock()
	defer b.mu.Unlock()
...
func (b *SimulatedBackend) EstimateGas(ctx context.Context, call trustmachine.CallMsg) (*big.Int, error) {
	b.mu.Lock()
	defer b.mu.Unlock()
...
func (b *SimulatedBackend) callContract(ctx context.Context, call trustmachine.CallMsg, block *types.Block, statedb *state.StateDB) ([]byte, *big.Int, error) {
