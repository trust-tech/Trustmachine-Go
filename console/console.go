...
type Config struct {
	...
	Client   *rpc.Client  // RPC client to execute Trustmachine requests through
...
func (c *Console) init(preload []string) error {
	...
	c.jsre.Set("jentrust", struct{}{})

	jentrustObj, _ := c.jsre.Get("jentrust")
	jentrustObj.Object().Set("send", bridge.Send)
	jentrustObj.Object().Set("sendAsync", bridge.Send)
	...
	if _, err := c.jsre.Run("var web3 = new Web3(jentrust);"); err != nil {
	...
	flatten := "var entrust = web3.entrust; var personal = web3.personal; "
	for api := range apis {
		if api == "web3" {
	...
	//c.jsre.Run(`var GlobalRegistrar = entrust.contract(` + registrar.GlobalRegistrarAbi + `);   registrar = GlobalRegistrar.at("` + registrar.GlobalRegistrarAddr + `");`)
	...
	if c.prompter != nil {
		...
		// Assign these method in the Console the original web3 callbacks. These will be called by the jentrust.*
		// methods after they got the password from the user and send the original web3 request to the backend.
		if obj := personal.Object(); obj != nil { // make sure the personal api is enabled over the interface
			if _, err = c.jsre.Run(`jentrust.unlockAccount = personal.unlockAccount;`); err != nil {
				return fmt.Errorf("personal.unlockAccount: %v", err)
			}
			if _, err = c.jsre.Run(`jentrust.newAccount = personal.newAccount;`); err != nil {
				return fmt.Errorf("personal.newAccount: %v", err)
			}
			if _, err = c.jsre.Run(`jentrust.sign = personal.sign;`); err != nil {
...
func (c *Console) AutoCompleteInput(line string, pos int) (string, []string, string) {
	...
	// E.g. in case of nested lines entrust.getBalance(entrust.coinb<tab><tab>
	start := pos - 1
	for ; start > 0; start-- {
...
// Welcome show summary of current Gotrust instance and some metadata about the
// console's available modules.
func (c *Console) Welcome() {
	// Print some generic Gotrust metadata
	fmt.Fprintf(c.printer, "Welcome to the Gotrust JavaScript console!\n\n")
	c.jsre.Run(`
		console.log("instance: " + web3.version.node);
		console.log("coinbase: " + entrust.coinbase);
		console.log("at block: " + entrust.blockNumber + " (" + new Date(1000 * entrust.getBlock(entrust.blockNumber).timestamp) + ")");
