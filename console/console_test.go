...
type tester struct {
	workspace string
	stack     *node.Node
	trustmachine  *entrust.Trustmachine
	console   *Console
...
func newTester(t *testing.T, confOverride func(*entrust.Config)) *tester {
	...
	// Create a networkless protocol stack and start an Trustmachine service within
	stack, err := node.New(&node.Config{DataDir: workspace, UseLightweightKDF: true, Name: testInstance})
	if err != nil {
		t.Fatalf("failed to create node: %v", err)
	}
	entrustConf := &entrust.Config{
		Genesis:   core.DevGenesisBlock(),
		Trustbase: common.HexToAddress(testAddress),
		PowTest:   true,
	}
	if confOverride != nil {
		confOverride(entrustConf)
	}
	if err = stack.Register(func(ctx *node.ServiceContext) (node.Service, error) { return entrust.New(ctx, entrustConf) }); err != nil {
		t.Fatalf("failed to register Trustmachine protocol: %v", err)
	}
