...
func newWorker(config *params.ChainConfig, engine consensus.Engine, coinbase common.Address, entrust Backend, mux *event.TypeMux) *worker {
	worker := &worker{
		config:         config,
		engine:         engine,
		entrust:            entrust,
		mux:            mux,
		chainDb:        entrust.ChainDb(),
		recv:           make(chan *Result, resultQueueSize),
		chain:          entrust.BlockChain(),
		proc:           entrust.BlockChain().Validator(),
		possibleUncles: make(map[common.Hash]*types.Block),
		coinbase:       coinbase,
		txQueue:        make(map[common.Hash]*types.Transaction),
		agents:         make(map[Agent]struct{}),
		unconfirmed:    newUnconfirmedBlocks(entrust.BlockChain(), 5),
...
func (self *worker) setTrustbase(addr common.Address) {
	self.mu.Lock()
...
func (self *worker) makeCurrent(parent *types.Block, header *types.Header) error {
	...
	wallets := self.entrust.AccountManager().Wallets()
...
func (self *worker) commitNewWork() {
	...
	pending, err := self.entrust.TxPool().Pending()
	if err != nil {
	...
	self.entrust.TxPool().RemoveBatch(work.failedTxs)
