...
// Package miner implements Trustmachine block creation and mining.
package miner
...
func New(entrust Backend, config *params.ChainConfig, mux *event.TypeMux, engine consensus.Engine) *Miner {
	miner := &Miner{
		entrust:      entrust,
		mux:      mux,
		engine:   engine,
		worker:   newWorker(config, engine, common.Address{}, entrust, mux),
		canStart: 1,
	}
	miner.Register(NewCpuAgent(entrust.BlockChain(), engine))
	go miner.update()
...
func (self *Miner) Start(coinbase common.Address) {
	atomic.StoreInt32(&self.shouldStart, 1)
	self.worker.setTrustbase(coinbase)
...
func (self *Miner) SetTrustbase(addr common.Address) {
	self.coinbase = addr
	self.worker.setTrustbase(addr)
}
