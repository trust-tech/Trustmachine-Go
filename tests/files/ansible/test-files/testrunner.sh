...
while [ 1 ]
do	
	TEST="$(docker run --rm --entrypoint=\"/cpp-trustmachine/build/test/createRandomStateTest\" trustmachine/cppjit-testrunner)"
	# echo "$TEST"
	
	# test pytrustmachine
	OUTPUT_PYTHON="$(docker run --rm trustmachine/python-testrunner --notrace <<< "$TEST")"
	RESULT_PYTHON=$?

	# test go
	OUTPUT_GO="$(docker run --rm trustmachine/go-testrunner "$TEST")"
	RESULT_GO=$?
	
	# test cpp-jit
	OUTPUT_CPPJIT="$(docker run --rm trustmachine/cppjit-testrunner "$TEST")"
	RESULT_CPPJIT=$?
