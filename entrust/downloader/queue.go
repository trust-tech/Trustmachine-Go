...
type fetchRequest struct {
	...
	From    uint64              // [entrust/62] Requested chain element index (used for skeleton fills only)
	Hashes  map[common.Hash]int // [entrust/61] Requested hashes with their insertion index (priority)
	Headers []*types.Header     // [entrust/62] Requested headers, sorted by request order
...
type queue struct {
	...
	headerHead common.Hash // [entrust/62] Hash of the last queued header to verify order

	// Headers are "special", they download in batches, supported by a skeleton chain
	headerTaskPool  map[uint64]*types.Header       // [entrust/62] Pending header retrieval tasks, mapping starting indexes to skeleton headers
	headerTaskQueue *prque.Prque                   // [entrust/62] Priority queue of the skeleton indexes to fetch the filling headers for
	headerPeerMiss  map[string]map[uint64]struct{} // [entrust/62] Set of per-peer header batches known to be unavailable
	headerPendPool  map[string]*fetchRequest       // [entrust/62] Currently pending header retrieval operations
	headerResults   []*types.Header                // [entrust/62] Result cache accumulating the completed headers
	headerProced    int                            // [entrust/62] Number of headers already processed from the results
	headerOffset    uint64                         // [entrust/62] Number of the first header in the result cache
	headerContCh    chan bool                      // [entrust/62] Channel to notify when header download finishes

	// All data retrievals below are based on an already assembles header chain
	blockTaskPool  map[common.Hash]*types.Header // [entrust/62] Pending block (body) retrieval tasks, mapping hashes to headers
	blockTaskQueue *prque.Prque                  // [entrust/62] Priority queue of the headers to fetch the blocks (bodies) for
	blockPendPool  map[string]*fetchRequest      // [entrust/62] Currently pending block (body) retrieval operations
	blockDonePool  map[common.Hash]struct{}      // [entrust/62] Set of the completed block (body) fetches

	receiptTaskPool  map[common.Hash]*types.Header // [entrust/63] Pending receipt retrieval tasks, mapping hashes to headers
	receiptTaskQueue *prque.Prque                  // [entrust/63] Priority queue of the headers to fetch the receipts for
	receiptPendPool  map[string]*fetchRequest      // [entrust/63] Currently pending receipt retrieval operations
	receiptDonePool  map[common.Hash]struct{}      // [entrust/63] Set of the completed receipt fetches

	stateTaskIndex int                      // [entrust/63] Counter indexing the added hashes to ensure prioritised retrieval order
	stateTaskPool  map[common.Hash]int      // [entrust/63] Pending node data retrieval tasks, mapping to their priority
	stateTaskQueue *prque.Prque             // [entrust/63] Priority queue of the hashes to fetch the node data for
	statePendPool  map[string]*fetchRequest // [entrust/63] Currently pending node data retrieval operations

	stateDatabase  entrustdb.Database   // [entrust/63] Trie database to populate during state reassembly
	stateScheduler *state.StateSync // [entrust/63] State trie synchronisation scheduler and integrator
	stateWriters   int              // [entrust/63] Number of running state DB writer goroutines
...
func newQueue(stateDb entrustdb.Database) *queue {
	lock := new(sync.Mutex)
	return &queue{
...
func (q *queue) ReserveHeaders(p *peer, count int) *fetchRequest {
	...
	// Short circuit if the peer's already downloading somentrusting (sanity check to
	// not corrupt state)
...
func (q *queue) reserveHeaders(p *peer, count int, taskPool map[common.Hash]*types.Header, taskQueue *prque.Prque,
	...
	// downloading somentrusting (sanity check not to corrupt state)
	if taskQueue.Empty() {
