...
// stateReq represents a batch of state fetch requests groupped togotruster into
...
func (d *Downloader) runStateSync(s *stateSync) *stateSync {
	...
	for {
		...
		case req := <-timeout:
			// If the peer is already requesting somentrusting else, ignore the stale timeout.
...
func (s *stateSync) loop() error {
	...
	for s.sched.Pending() > 0 {
		...
		// Tasks assigned, wait for somentrusting to happen
		select {
		case <-newPeer:
...
func (s *stateSync) process(req *stateReq) (bool, error) {
	...
	for hash, task := range req.tasks {
		// If the node did deliver somentrusting, missing items may be due to a protocol
