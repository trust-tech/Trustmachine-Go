...
var (
	...
	maxQueuedHeaders  = 32 * 1024 // [entrust/62] Maximum number of headers to queue for import (DOS protection)
...
type Downloader struct {
	...
	// Channels
	newPeerCh     chan *peer
	headerCh      chan dataPack        // [entrust/62] Channel receiving inbound block headers
	bodyCh        chan dataPack        // [entrust/62] Channel receiving inbound block bodies
	receiptCh     chan dataPack        // [entrust/63] Channel receiving inbound receipts
	stateCh       chan dataPack        // [entrust/63] Channel receiving inbound node state data
	bodyWakeCh    chan bool            // [entrust/62] Channel to signal the block body fetcher of new tasks
	receiptWakeCh chan bool            // [entrust/63] Channel to signal the receipt fetcher of new tasks
	stateWakeCh   chan bool            // [entrust/63] Channel to signal the state fetcher of new tasks
	headerProcCh  chan []*types.Header // [entrust/62] Channel to feed the header processor new tasks
...
func New(mode SyncMode, stateDb entrustdb.Database, mux *event.TypeMux, hasHeader headerCheckFn, hasBlockAndState blockAndStateCheckFn,
...
// these are zero.
func (d *Downloader) Progress() trustmachine.SyncProgress {
...
func (d *Downloader) syncWithPeer(p *peer, hash common.Hash, td *big.Int) (err error) {
	...
	log.Debug("Synchronising with the network", "peer", p.id, "entrust", p.version, "head", hash, "td", td, "mode", d.mode)
...
func (d *Downloader) fetchHeight(p *peer) (*types.Header, error) {
	...
	for {
		...
		case packet := <-d.headerCh:
			...
			// Make sure the peer actually gave somentrusting valid
	...
	for {
		...
		case packet := <-deliveryCh:
			...
			if peer := d.peers.Peer(packet.PeerId()); peer != nil {
				...
				// Unless a peer delivered somentrusting completely else than requested (usually
	...
	for {
		...
		case headers := <-d.headerProcCh:
			...
			if len(headers) == 0 {
				...
				// L: Request new headers up from 11 (R's TD was higher, it must have somentrusting)
				// R: Nothing to give
				if d.mode != LightSync {
				...
				// peer gave us somentrusting useful, we're already happy/progressed (above check).
			...
			for len(headers) > 0 {
				// Terminate if somentrusting failed in between processing chunks
				select {
				case <-d.cancelCh:
					return errCancelHeaderProcessing
				default:
