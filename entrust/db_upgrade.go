...
// been safely stopped.
func upgradeSequentialKeys(db entrustdb.Database) (stopFn func()) {
...
func upgradeSequentialCanonicalNumbers(db entrustdb.Database, stopFn func() bool) (error, bool) {
	prefix := []byte("block-num-")
	it := db.(*entrustdb.LDBDatabase).NewIterator()
	defer func() {
		it.Release()
	}()
	it.Seek(prefix)
...
// if successful.
func upgradeSequentialBlocks(db entrustdb.Database, stopFn func() bool) (error, bool) {
	prefix := []byte("block-")
	it := db.(*entrustdb.LDBDatabase).NewIterator()
	defer func() {
		it.Release()
...
func upgradeSequentialOrphanedReceipts(db entrustdb.Database, stopFn func() bool) (error, bool) {
...
func upgradeSequentialBlockData(db entrustdb.Database, hash []byte) error {
...
func addMipmapBloomBins(db entrustdb.Database) (err error) {
