...
func testGetNodeData(t *testing.T, protocol int) {
	...
	hashes := []common.Hash{}
	for _, key := range pm.chaindb.(*entrustdb.MemDatabase).Keys() {
	...
	statedb, _ := entrustdb.NewMemDatabase()
...
// Tests that post entrust protocol handshake, DAO fork-enabled clients also execute
...
func testDAOChallenge(t *testing.T, localForked, remoteForked bool, timeout bool) {
	...
	peer, _ := newTestPeer("peer", entrust63, pm, true)
	defer peer.close()
