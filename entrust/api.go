...
import (
	...
	"github.com/ThePleasurable/go-trustmachine/miner"
...
// PublicTrustmachineAPI provides an API to access Trustmachine full node-related
// information.
type PublicTrustmachineAPI struct {
	e *Trustmachine
}

// NewPublicTrustmachineAPI creates a new Trustuem protocol API for full nodes.
func NewPublicTrustmachineAPI(e *Trustmachine) *PublicTrustmachineAPI {
	return &PublicTrustmachineAPI{e}
}

// Trustbase is the address that mining rewards will be send to
func (api *PublicTrustmachineAPI) Trustbase() (common.Address, error) {
	return api.e.Trustbase()
}

// Coinbase is the address that mining rewards will be send to (alias for Trustbase)
func (api *PublicTrustmachineAPI) Coinbase() (common.Address, error) {
	return api.Trustbase()
}

// Hashrate returns the POW hashrate
func (api *PublicTrustmachineAPI) Hashrate() hexutil.Uint64 {
...
func NewPublicMinerAPI(e *Trustmachine) *PublicMinerAPI {
...
func NewPrivateMinerAPI(e *Trustmachine) *PrivateMinerAPI {
...
// SetTrustbase sets the trustbase of the miner
func (api *PrivateMinerAPI) SetTrustbase(trustbase common.Address) bool {
	api.e.SetTrustbase(trustbase)
	return true
}
...
// PrivateAdminAPI is the collection of Trustuem full node-related APIs
...
// admin methods of the Trustmachine service.
func NewPrivateAdminAPI(entrust *Trustmachine) *PrivateAdminAPI {
	return &PrivateAdminAPI{entrust: entrust}
}
...
func (api *PrivateAdminAPI) ExportChain(file string) (bool, error) {
	...
	if err := api.entrust.BlockChain().Export(writer); err != nil {
		return false, err
	}
	return true, nil
}
...
func (api *PrivateAdminAPI) ImportChain(file string) (bool, error) {
	...
	for batch := 0; ; batch++ {
		...
		if hasAllBlocks(api.entrust.BlockChain(), blocks) {
			blocks = blocks[:0]
			continue
		}
		// Import the batch and reset the buffer
		if _, err := api.entrust.BlockChain().InsertChain(blocks); err != nil {
...
// PublicDebugAPI is the collection of Trustuem full node APIs exposed
...
// related public debug methods of the Trustmachine service.
func NewPublicDebugAPI(entrust *Trustmachine) *PublicDebugAPI {
	return &PublicDebugAPI{entrust: entrust}
}
...
func (api *PublicDebugAPI) DumpBlock(blockNr rpc.BlockNumber) (state.Dump, error) {
	if blockNr == rpc.PendingBlockNumber {
		...
		_, stateDb := api.entrust.miner.Pending()
		return stateDb.RawDump(), nil
	}
	var block *types.Block
	if blockNr == rpc.LatestBlockNumber {
		block = api.entrust.blockchain.CurrentBlock()
	} else {
		block = api.entrust.blockchain.GetBlockByNumber(uint64(blockNr))
	}
	if block == nil {
		return state.Dump{}, fmt.Errorf("block #%d not found", blockNr)
	}
	stateDb, err := api.entrust.BlockChain().StateAt(block.Root())
	if err != nil {
...
// PrivateDebugAPI is the collection of Trustuem full node APIs exposed over
...
// private debug methods of the Trustmachine service.
func NewPrivateDebugAPI(config *params.ChainConfig, entrust *Trustmachine) *PrivateDebugAPI {
	return &PrivateDebugAPI{config: config, entrust: entrust}
}
...
type BlockTraceResult struct {
	...
	StructLogs []entrustapi.StructLogRes `json:"structLogs"`
...
func (api *PrivateDebugAPI) TraceBlock(blockRlp []byte, config *vm.LogConfig) BlockTraceResult {
	...
	return BlockTraceResult{
		Validated:  validated,
		StructLogs: entrustapi.FormatLogs(logs),
...
func (api *PrivateDebugAPI) TraceBlockByNumber(blockNr rpc.BlockNumber, config *vm.LogConfig) BlockTraceResult {
	...
	case rpc.PendingBlockNumber:
		...
		block = api.entrust.miner.PendingBlock()
...
func (api *PrivateDebugAPI) TraceBlockByHash(hash common.Hash, config *vm.LogConfig) BlockTraceResult {
	// Fetch the block that we aim to reprocess
	block := api.entrust.BlockChain().GetBlockByHash(hash)
	if block == nil {
...
func (api *PrivateDebugAPI) traceBlock(block *types.Block, logConfig *vm.LogConfig) (bool, []vm.StructLog, error) {
	...
	var (
		blockchain = api.entrust.BlockChain()
	...
	if err := api.entrust.engine.VerifyHeader(blockchain, block.Header(), true); err != nil {
...
func (api *PrivateDebugAPI) TraceTransaction(ctx context.Context, txHash common.Hash, config *TraceArgs) (interface{}, error) {
	...
	if config != nil && config.Tracer != nil {
		...
		var err error
		if tracer, err = entrustapi.NewJavascriptTracer(*config.Tracer); err != nil {
			return nil, err
		}
		...
		go func() {
			<-deadlineCtx.Done()
			tracer.(*entrustapi.JavascriptTracer).Stop(&timeoutError{})
		}()
		defer cancel()
	} else if config == nil {
	...
	tx, blockHash, _, txIndex := core.GetTransaction(api.entrust.ChainDb(), txHash)
	if tx == nil {
	...
	case *vm.StructLogger:
		return &entrustapi.ExecutionResult{
			Gas:         gas,
			ReturnValue: fmt.Sprintf("%x", ret),
			StructLogs:  entrustapi.FormatLogs(tracer.StructLogs()),
		}, nil
	case *entrustapi.JavascriptTracer:
...
func (api *PrivateDebugAPI) computeTxEnv(blockHash common.Hash, txIndex int) (core.Message, vm.Context, *state.StateDB, error) {
	...
	block := api.entrust.BlockChain().GetBlockByHash(blockHash)
	if block == nil {
		return nil, vm.Context{}, nil, fmt.Errorf("block %x not found", blockHash)
	}
	parent := api.entrust.BlockChain().GetBlock(block.ParentHash(), block.NumberU64()-1)
	if parent == nil {
		return nil, vm.Context{}, nil, fmt.Errorf("block parent %x not found", block.ParentHash())
	}
	statedb, err := api.entrust.BlockChain().StateAt(parent.Root())
	if err != nil {
	...
	for idx, tx := range txs {
		...
		context := core.NewEVMContext(msg, block.Header(), api.entrust.BlockChain(), nil)
		if idx == txIndex {
...
func (api *PrivateDebugAPI) Preimage(ctx context.Context, hash common.Hash) (hexutil.Bytes, error) {
	db := core.PreimageTable(api.entrust.ChainDb())
...
func (api *PrivateDebugAPI) GetBadBlocks(ctx context.Context) ([]core.BadBlockArgs, error) {
	return api.entrust.BlockChain().BadBlocks()
}
