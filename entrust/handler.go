...
import (
	...
	"github.com/ThePleasurable/go-trustmachine/consensus/misc"
	...
	"github.com/ThePleasurable/go-trustmachine/entrust/fetcher"
...
// NewProtocolManager returns a new trustmachine sub protocol manager. The Trustmachine sub protocol manages peers capable
// with the trustmachine network.
func NewProtocolManager(config *params.ChainConfig, mode downloader.SyncMode, networkId uint64, maxPeers int, mux *event.TypeMux, txpool txPool, engine consensus.Engine, blockchain *core.BlockChain, chaindb entrustdb.Database) (*ProtocolManager, error) {
	...
	for i, version := range ProtocolVersions {
		...
		if mode == downloader.FastSync && version < entrust63 {
			continue
		}
...
func (pm *ProtocolManager) removePeer(id string) {
	...
	log.Debug("Removing Trustmachine peer", "peer", id)

	// Unregister the peer from the downloader and Trustmachine peer set
...
func (pm *ProtocolManager) Stop() {
	log.Info("Stopping Trustmachine protocol")
	...
	log.Info("Trustmachine protocol stopped")
}
...
// handle is the callback invoked to manage the life cycle of an entrust peer. When
// this function terminates, the peer is disconnected.
func (pm *ProtocolManager) handle(p *peer) error {
	if pm.peers.Len() >= pm.maxPeers {
		return p2p.DiscTooManyPeers
	}
	p.Log().Debug("Trustmachine peer connected", "name", p.Name())

	// Execute the Trustmachine handshake
	td, head, genesis := pm.blockchain.Status()
	if err := p.Handshake(pm.networkId, td, head, genesis); err != nil {
		p.Log().Debug("Trustmachine handshake failed", "err", err)
		return err
	}
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("Trustmachine peer registration failed", "err", err)
		return err
	}
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Trustmachine message handling failed", "err", err)
			return err
		}
	}
}
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	case p.version >= entrust63 && msg.Code == GetNodeDataMsg:
		// Decode the retrieval message
		msgStream := rlp.NewStream(msg.Payload, uint64(msg.Size))
		if _, err := msgStream.List(); err != nil {
			return err
	...
	case p.version >= entrust63 && msg.Code == NodeDataMsg:
	...
	case p.version >= entrust63 && msg.Code == GetReceiptsMsg:
		// Decode the retrieval message
		msgStream := rlp.NewStream(msg.Payload, uint64(msg.Size))
		if _, err := msgStream.List(); err != nil {
			return err
	...
	case p.version >= entrust63 && msg.Code == ReceiptsMsg:
...
// EntrustNodeInfo represents a short summary of the Trustmachine sub-protocol metadata known
// about the host peer.
type EntrustNodeInfo struct {
	Network    uint64      `json:"network"`    // Trustmachine network ID (1=Frontier, 2=Morden, Ropsten=3)
...
func (self *ProtocolManager) NodeInfo() *EntrustNodeInfo {
	currentBlock := self.blockchain.CurrentBlock()
	return &EntrustNodeInfo{
