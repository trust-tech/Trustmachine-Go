...
// Package state provides a caching layer atop the Trustmachine state trie.
package state
...
// StateDBs within the trustmachine protocol are used to store anything
...
func New(root common.Hash, db entrustdb.Database) (*StateDB, error) {
...
//
// Carrying over the balance ensures that Trust doesn't disappear.
...
func (s *StateDB) CommitBatch(deleteEmptyObjects bool) (root common.Hash, batch entrustdb.Batch) {
	batch = s.db.NewBatch()
