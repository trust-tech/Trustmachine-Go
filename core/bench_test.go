...
// genTxRing returns a block generator that sends trust in a ring
...
func benchInsertChain(b *testing.B, disk bool, gen func(int, *BlockGen)) {
	...
	var db entrustdb.Database
	if !disk {
		db, _ = entrustdb.NewMemDatabase()
	} else {
		dir, err := ioutil.TempDir("", "entrust-core-bench")
		if err != nil {
			b.Fatalf("cannot create temporary directory: %v", err)
		}
		defer os.RemoveAll(dir)
		db, err = entrustdb.NewLDBDatabase(dir, 128, 128)
		if err != nil {
	...
	chainman, _ := NewBlockChain(db, gspec.Config, entrustash.NewFaker(), evmux, vm.Config{})
	defer chainman.Stop()
	b.ReportAllocs()
	b.ResetTimer()
...
// into a database.
func makeChainForBench(db entrustdb.Database, full bool, count uint64) {
...
func benchWriteChain(b *testing.B, full bool, count uint64) {
	for i := 0; i < b.N; i++ {
		dir, err := ioutil.TempDir("", "entrust-chain-bench")
		if err != nil {
			b.Fatalf("cannot create temporary directory: %v", err)
		}
		db, err := entrustdb.NewLDBDatabase(dir, 128, 1024)
		if err != nil {
...
func benchReadChain(b *testing.B, full bool, count uint64) {
	...
	for i := 0; i < b.N; i++ {
		...
		chain, err := NewBlockChain(db, params.TestChainConfig, entrustash.NewFaker(), new(event.TypeMux), vm.Config{})
		if err != nil {
