...
func newTestBlockChain(fake bool) *BlockChain {
	...
	gspec.MustCommit(db)
	engine := entrustash.NewFullFaker()
	if !fake {
		engine = entrustash.NewTester()
	}
...
func testReorgBadHashes(t *testing.T, full bool) {
	...
	ncm, err := NewBlockChain(bc.chainDb, bc.config, entrustash.NewFaker(), new(event.TypeMux), vm.Config{})
	if err != nil {
...
func testInsertNonceError(t *testing.T, full bool) {
	for i := 1; i < 25 && !t.Failed(); i++ {
		...
		if full {
			...
			blockchain.engine = entrustash.NewFakeFailer(failNum)
...
func TestFastVsFullChains(t *testing.T) {
	...
	var (
		gendb, _ = entrustdb.NewMemDatabase()
	...
	archiveDb, _ := entrustdb.NewMemDatabase()
	gspec.MustCommit(archiveDb)
	archive, _ := NewBlockChain(archiveDb, gspec.Config, entrustash.NewFaker(), new(event.TypeMux), vm.Config{})
	...
	fastDb, _ := entrustdb.NewMemDatabase()
	gspec.MustCommit(fastDb)
	fast, _ := NewBlockChain(fastDb, gspec.Config, entrustash.NewFaker(), new(event.TypeMux), vm.Config{})
...
func TestLightVsFastVsFullChainHeads(t *testing.T) {
	...
	lightDb, _ := entrustdb.NewMemDatabase()
	gspec.MustCommit(lightDb)

	light, _ := NewBlockChain(lightDb, gspec.Config, entrustash.NewFaker(), new(event.TypeMux), vm.Config{})
...
func TestChainTxReorgs(t *testing.T) {
	...
	evmux := &event.TypeMux{}
	blockchain, _ := NewBlockChain(db, gspec.Config, entrustash.NewFaker(), evmux, vm.Config{})
...
func TestEIP155Transition(t *testing.T) {
	...
	blockchain, _ := NewBlockChain(db, gspec.Config, entrustash.NewFaker(), &mux, vm.Config{})
