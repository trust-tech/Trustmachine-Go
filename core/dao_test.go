...
func TestDAOForkRangeExtradata(t *testing.T) {
	...
	proDb, _ := entrustdb.NewMemDatabase()
	gspec.MustCommit(proDb)
	proConf := &params.ChainConfig{HomesteadBlock: big.NewInt(0), DAOForkBlock: forkBlock, DAOForkSupport: true}
	proBc, _ := NewBlockChain(proDb, proConf, entrustash.NewFaker(), new(event.TypeMux), vm.Config{})

	conDb, _ := entrustdb.NewMemDatabase()
	gspec.MustCommit(conDb)
	conConf := &params.ChainConfig{HomesteadBlock: big.NewInt(0), DAOForkBlock: forkBlock, DAOForkSupport: false}
	conBc, _ := NewBlockChain(conDb, conConf, entrustash.NewFaker(), new(event.TypeMux), vm.Config{})
	...
	for i := int64(0); i < params.DAOForkExtraRange.Int64(); i++ {
		...
		db, _ = entrustdb.NewMemDatabase()
		gspec.MustCommit(db)
		bc, _ := NewBlockChain(db, conConf, entrustash.NewFaker(), new(event.TypeMux), vm.Config{})
		...
		db, _ = entrustdb.NewMemDatabase()
		gspec.MustCommit(db)
		bc, _ = NewBlockChain(db, proConf, entrustash.NewFaker(), new(event.TypeMux), vm.Config{})
