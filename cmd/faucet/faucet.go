...
// faucet is a Trust faucet backed by a light client.
package main
...
var (
	...
	entrustPortFlag = flag.Int("entrustport", 30303, "Listener port for the devp2p connection")
	bootFlag    = flag.String("bootnodes", "", "Comma separated bootnode enode URLs to seed with")
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the Trustmachine protocol")
	statsFlag   = flag.String("entruststats", "", "Entruststats network monitoring auth string")

	netnameFlag = flag.String("faucet.name", "", "Network name to assign to the faucet")
	payoutFlag  = flag.Int("faucet.amount", 1, "Number of Trusts to pay out per user request")
	...
	logFlag = flag.Int("loglevel", 3, "Log level to use for Trustmachine and the faucet")
)

var (
	trust = new(big.Int).Exp(big.NewInt(10), big.NewInt(18), nil)
)
...
func main() {
	...
	for i := 0; i < *tiersFlag; i++ {
		...
		amounts[i] = fmt.Sprintf("%s Trusts", strconv.FormatFloat(amount, 'f', -1, 64))
		if amount == 1 {
	...
	faucet, err := newFaucet(genesis, *entrustPortFlag, enodes, *netFlag, *statsFlag, ks, website.Bytes())
	if err != nil {
...
type request struct {
	...
	Account  common.Address     `json:"account"`  // Trustmachine address being funded
	Time     time.Time          `json:"time"`     // Timestamp when te request was accepted
	Tx       *types.Transaction `json:"tx"`       // Transaction funding the account
}

// faucet represents a crypto faucet backed by an Trustmachine light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // Trustmachine protocol stack
	client *entrustclient.Client   // Client connection to the Trustmachine chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the Trustmachine light client protocol
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		cfg := entrust.DefaultConfig
	...
	// Assemble the entruststats monitoring and reporting service'
	if stats != "" {
		if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
			var serv *les.LightTrustmachine
			ctx.Service(&serv)
			return entruststats.New(stats, nil, serv)
		}); err != nil {
			return nil, err
		}
	}
	...
	client := entrustclient.NewClient(api)
...
// close terminates the Trustmachine connection and tears down the faucet.
...
// apiHandler handles requests for Trust grants and transaction statuses.
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	websocket.JSON.Send(conn, map[string]interface{}{
		"funds":    balance.Div(balance, trust),
		"funded":   nonce,
	...
	for {
		...
		// Iterate over all the files and look for Trustmachine addresses
		...
		if address == (common.Address{}) {
			websocket.JSON.Send(conn, map[string]string{"error": "No Trustmachine address found to fund"})
			continue
		}
		...
		if timeout = f.timeouts[gist.Owner.Login]; time.Now().After(timeout) {
			...
			amount := new(big.Int).Mul(big.NewInt(int64(*payoutFlag)), trust)
...
func (f *faucet) loop() {
	...
	for {
		...
		case head := <-heads:
			...
			balance, _ := f.client.BalanceAt(context.Background(), f.account.Address, nil)
			balance = new(big.Int).Div(balance, trust)
