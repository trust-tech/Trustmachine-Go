...
func testDAOForkBlockNewChain(t *testing.T, test int, genesis string, expectBlock *big.Int, expectVote bool) {
	...
	// Start a Gotrust instance with the requested flags set and immediately terminate
	if genesis != "" {
		...
		gotrust := runGotrust(t, append(args, []string{"--exec", "2+2", "console"}...)...)
		gotrust.cmd.Wait()
	}
	// Retrieve the DAO config flag from the database
	path := filepath.Join(datadir, "gotrust", "chaindata")
	db, err := entrustdb.NewLDBDatabase(path, 0, 0)
	if err != nil {
