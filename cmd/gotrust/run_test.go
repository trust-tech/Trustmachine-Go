...
func tmpdir(t *testing.T) string {
	dir, err := ioutil.TempDir("", "gotrust-test")
	if err != nil {
		t.Fatal(err)
	}
	return dir
...
func init() {
	// Run the app if we're the child process for runGotrust.
	if os.Getenv("GOTRUST_TEST_CHILD") != "" {
...
// spawns gotrust with the given command line args. If the args don't set --datadir, the
// child g gets a temporary data directory.
func runGotrust(t *testing.T, args ...string) *testgotrust {
	tt := &testgotrust{T: t, Executable: os.Args[0]}
	for i, arg := range args {
		...
		case arg == "-trustbase" || arg == "--trustbase":
			if i < len(args)-1 {
				tt.Trustbase = args[i+1]
			}
		}
	}
	if tt.Datadir == "" {
		...
		// Remove the temporary datadir if somentrusting fails below.
		defer func() {
			if t.Failed() {
	...
	// Boot "gotrust". This actually runs the test binary but the init function
	// will prevent any tests from running.
	tt.stderr = &testlogger{t: t}
	tt.cmd = exec.Command(os.Args[0], args...)
	tt.cmd.Env = append(os.Environ(), "GOTRUST_TEST_CHILD=1")
...
//
//     gotrust.expect(`Passphrase: {{.InputLine "password"}}`)
func (tt *testgotrust) InputLine(s string) string {
	io.WriteString(tt.stdin, s+"\n")
	return ""
}

func (tt *testgotrust) setTemplateFunc(name string, fn interface{}) {
	if tt.Func == nil {
...
// before matching.
func (tt *testgotrust) expect(tplsource string) {
...
func (tt *testgotrust) matchExactOutput(want []byte) error {
...
// after expectRegexp.
func (tt *testgotrust) expectRegexp(resource string) (*regexp.Regexp, []string) {
	var (
...
func (tt *testgotrust) expectExit() {
	var output []byte
...
// returned.
func (tt *testgotrust) stderrText() string {
	tt.stderr.mu.Lock()
	defer tt.stderr.mu.Unlock()
	return tt.stderr.buf.String()
}

func (tt *testgotrust) withKillTimeout(fn func()) {
