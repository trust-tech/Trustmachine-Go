...
var nodeDockerfile = `
FROM trustmachine/client-go:alpine-develop
...
{{end}}
RUN \
  echo '/gotrust init /genesis.json' > gotrust.sh && \{{if .Unlock}}
	echo 'mkdir -p /root/.trustmachine/keystore/ && cp /signer.json /root/.trustmachine/keystore/' >> gotrust.sh && \{{end}}
	echo $'/gotrust --networkid {{.NetworkID}} --cache 512 --port {{.Port}} --maxpeers {{.Peers}} {{.LightFlag}} --entruststats \'{{.Entruststats}}\' {{if .BootV4}}--bootnodesv4 {{.BootV4}}{{end}} {{if .BootV5}}--bootnodesv5 {{.BootV5}}{{end}} {{if .Trustbase}}--trustbase {{.Trustbase}} --mine{{end}}{{if .Unlock}}--unlock 0 --password /signer.pass --mine{{end}} --targetgaslimit {{.GasTarget}} --gasprice {{.GasPrice}}' >> gotrust.sh

ENTRYPOINT ["/bin/sh", "gotrust.sh"]
`

// nodeComposefile is the docker-compose.yml file required to deploy and maintain
// an Trustmachine node (bootnode or miner for now).
var nodeComposefile = `
version: '2'
services:
  {{.Type}}:
    ...
    volumes:
      - {{.Datadir}}:/root/.trustmachine
    environment:
      - FULL_PORT={{.FullPort}}/tcp
      - LIGHT_PORT={{.LightPort}}/udp
      - TOTAL_PEERS={{.TotalPeers}}
      - LIGHT_PEERS={{.LightPeers}}
      - STATS_NAME={{.Entruststats}}
      - MINER_NAME={{.Trustbase}}
...
func deployNode(client *sshClient, network string, bootv4, bootv5 []string, config *nodeInfos) ([]byte, error) {
	...
	template.Must(template.New("").Parse(nodeDockerfile)).Execute(dockerfile, map[string]interface{}{
		...
		"Entruststats":  config.entruststats,
	...
	template.Must(template.New("").Parse(nodeComposefile)).Execute(composefile, map[string]interface{}{
		...
		"Entruststats":   config.entruststats[:strings.Index(config.entruststats, ":")],
		"Trustbase":  config.trustbase,
...
func (info *nodeInfos) String() string {
	...
	return fmt.Sprintf("port=%d%s, datadir=%s, peers=%d, lights=%d, entruststats=%s, gastarget=%0.3f MGas, gasprice=%0.3f GWei",
		info.portFull, discv5, info.datadir, info.peersTotal, info.peersLight, info.entruststats, info.gasTarget, info.gasPrice)
}
...
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	var out []byte
	if out, err = client.Run(fmt.Sprintf("docker exec %s_%s_1 /gotrust --exec admin.nodeInfo.id attach", network, kind)); err != nil {
	...
	stats := &nodeInfos{
		genesis:    genesis,
		datadir:    infos.volumes["/root/.trustmachine"],
		portFull:   infos.portmap[infos.envvars["FULL_PORT"]],
		portLight:  infos.portmap[infos.envvars["LIGHT_PORT"]],
		peersTotal: totalPeers,
		peersLight: lightPeers,
		entruststats:   infos.envvars["STATS_NAME"],
		trustbase:  infos.envvars["MINER_NAME"],
		keyJSON:    keyJSON,
		keyPass:    keyPass,
		gasTarget:  gasTarget,
		gasPrice:   gasPrice,
