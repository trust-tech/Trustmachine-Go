...
// config contains all the configurations needed by puppentrust that should be saved
// between sessions.
type config struct {
	...
	entruststats  string        // Entruststats settings to cache for node deploys
...
func (c config) flush() {
	...
	if err := ioutil.WriteFile(c.path, out, 0644); err != nil {
		log.Warn("Failed to save puppentrust configs", "file", c.path, "err", err)
	}
}
...
type wizard struct {
	...
	services map[string][]string   // Trustmachine services known to be running on servers
...
// converts it to an Trustmachine address. If an empty line is entered, the default
// value is returned.
