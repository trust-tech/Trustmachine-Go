...
func (w *wizard) networkStats(tips bool) {
	...
	for server, pubkey := range w.conf.Servers {
		...
		logger.Debug("Checking for entruststats availability")
		if infos, err := checkEntruststats(client, w.network); err != nil {
			if err != ErrServiceUnknown {
				services["entruststats"] = err.Error()
			}
		} else {
			services["entruststats"] = infos.String()
			protips.entruststats = infos.config
		}
	...
	if protips.entruststats != "" {
		w.conf.entruststats = protips.entruststats
	}
...
func (p *protips) print(network string) {
	...
	fullinit, lightinit := "", ""
	if p.genesis != "" {
		fullinit = fmt.Sprintf("gotrust --datadir=$HOME/.%s init %s.json && ", network, network)
		lightinit = fmt.Sprintf("gotrust --datadir=$HOME/.%s --light init %s.json && ", network, network)
	}
	// If an entruststats server is available, add the entruststats flag
	statsflag := ""
	if p.entruststats != "" {
		if strings.Contains(p.entruststats, " ") {
			statsflag = fmt.Sprintf(` --entruststats="yournode:%s"`, p.entruststats)
		} else {
			statsflag = fmt.Sprintf(` --entruststats=yournode:%s`, p.entruststats)
		}
	}
	...
	tips = append(tips, fmt.Sprintf("%sgotrust --networkid=%d --datadir=$HOME/.%s --cache=1024%s%s", fullinit, p.network, network, statsflag, bootflagFull))

	tasks = append(tasks, "Run a full node with recent data only")
	tips = append(tips, fmt.Sprintf("%sgotrust --networkid=%d --datadir=$HOME/.%s --cache=512 --fast%s%s", fullinit, p.network, network, statsflag, bootflagFull))

	tasks = append(tasks, "Run a light node with on demand retrievals")
	tips = append(tips, fmt.Sprintf("%sgotrust --networkid=%d --datadir=$HOME/.%s --light%s%s", lightinit, p.network, network, statsflag, bootflagLight))

	tasks = append(tasks, "Run an embedded node with constrained memory")
	tips = append(tips, fmt.Sprintf("%sgotrust --networkid=%d --datadir=$HOME/.%s --cache=32 --light%s%s", lightinit, p.network, network, statsflag, bootflagLight))
