...
func (w *wizard) deployDashboard() {
	...
	for _, service := range []string{"entruststats", "explorer", "wallet", "faucet"} {
	...
	// If we have entruststats running, ask whether to make the secret public or not
	var entruststats bool
	if w.conf.entruststats != "" {
		fmt.Println()
		fmt.Println("Include entruststats secret on dashboard (y/n)? (default = yes)")
		entruststats = w.readDefaultString("y") == "y"
	}
	// Try to deploy the dashboard container on the host
	if out, err := deployDashboard(client, w.network, infos.port, infos.host, listing, &w.conf, entruststats); err != nil {
