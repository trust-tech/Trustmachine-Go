...
func (w *wizard) deployNode(boot bool) {
	...
	if w.conf.entruststats == "" {
		log.Error("No entruststats server configured")
		return
	}
	...
	fmt.Println()
	if infos.entruststats == "" {
		fmt.Printf("What should the node be called on the stats page?\n")
		infos.entruststats = w.readString() + ":" + w.conf.entruststats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.entruststats)
		infos.entruststats = w.readDefaultString(infos.entruststats) + ":" + w.conf.entruststats
	}
	// If the node is a miner/signer, load up needed credentials
	if !boot {
		if w.conf.genesis.Config.Entrustash != nil {
			// Entrustash based miners only need an trustbase to mine against
			fmt.Println()
			if infos.trustbase == "" {
				fmt.Printf("What address should the miner user?\n")
				for {
					if address := w.readAddress(); address != nil {
						infos.trustbase = address.Hex()
						break
					}
				}
			} else {
				fmt.Printf("What address should the miner user? (default = %s)\n", infos.trustbase)
				infos.trustbase = w.readDefaultAddress(common.HexToAddress(infos.trustbase)).Hex()
			}
	...
	if out, err := deployNode(client, w.network, w.conf.bootFull, w.conf.bootLight, infos); err != nil {
		log.Error("Failed to deploy Trustmachine node container", "err", err)
		if len(out) > 0 {
			fmt.Printf("%s\n", out)
		}
		return
