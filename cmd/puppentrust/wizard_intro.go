...
// makeWizard creates and returns a new puppentrust wizard.
...
// setting up a new or managing an existing Trustmachine private network.
func (w *wizard) run() {
	fmt.Println("+-----------------------------------------------------------+")
	fmt.Println("| Welcome to puppentrust, your Trustmachine private network manager |")
	fmt.Println("|                                                           |")
	fmt.Println("| This tool lets you create a new Trustmachine network down to  |")
	fmt.Println("| the genesis block, bootnodes, miners and entruststats servers |")
	fmt.Println("| without the hassle that it would normally entail.         |")
	fmt.Println("|                                                           |")
	fmt.Println("| Puppentrust uses SSH to dial in to remote servers, and builds |")
	...
	log.Info("Administering Trustmachine network", "name", w.network)

	// Load initial configurations and connect to all live servers
	w.conf.path = filepath.Join(os.Getenv("HOME"), ".puppentrust", w.network)
	...
	for {
		...
		default:
			log.Error("That's not somentrusting I can do")
		}
	}
}
