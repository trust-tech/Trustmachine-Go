...
func (w *wizard) deployFaucet() {
	...
	fmt.Println()
	fmt.Printf("How many Trusts to release per request? (default = %d)\n", infos.amount)
	...
	fmt.Println()
	if infos.node.entruststats == "" {
		fmt.Printf("What should the node be called on the stats page?\n")
		infos.node.entruststats = w.readString() + ":" + w.conf.entruststats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.node.entruststats)
		infos.node.entruststats = w.readDefaultString(infos.node.entruststats) + ":" + w.conf.entruststats
	}
