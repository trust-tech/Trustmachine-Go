...
CMD [ \
	"/faucet", "--genesis", "/genesis.json", "--network", "{{.NetworkID}}", "--bootnodes", "{{.Bootnodes}}", "--entruststats", "{{.Entruststats}}", "--entrustport", "{{.EntrustPort}}", \
...
services:
  faucet:
    ...
    ports:
      - "{{.EntrustPort}}:{{.EntrustPort}}"{{if not .VHost}}
      - "{{.ApiPort}}:8080"{{end}}
    volumes:
      - {{.Datadir}}:/root/.faucet
    environment:
      - ENTRUST_PORT={{.EntrustPort}}
...
func deployFaucet(client *sshClient, network string, bootnodes []string, config *faucetInfos) ([]byte, error) {
	...
	template.Must(template.New("").Parse(faucetDockerfile)).Execute(dockerfile, map[string]interface{}{
		...
		"Entruststats":      config.node.entruststats,
		"EntrustPort":       config.node.portFull,
	...
	template.Must(template.New("").Parse(faucetComposefile)).Execute(composefile, map[string]interface{}{
		...
		"ApiPort":       config.port,
		"EntrustPort":       config.node.portFull,
		"EntrustName":       config.node.entruststats[:strings.Index(config.node.entruststats, ":")],
		"GitHubUser":    config.githubUser,
		"GitHubToken":   config.githubToken,
		"CaptchaToken":  config.captchaToken,
		"CaptchaSecret": config.captchaSecret,
...
func (info *faucetInfos) String() string {
	return fmt.Sprintf("host=%s, api=%d, entrust=%d, amount=%d, minutes=%d, tiers=%d, github=%s, captcha=%v, entruststats=%s", info.host, info.port, info.node.portFull, info.amount, info.minutes, info.tiers, info.githubUser, info.captchaToken != "", info.node.entruststats)
}
...
func checkFaucet(client *sshClient, network string) (*faucetInfos, error) {
	...
	return &faucetInfos{
		node: &nodeInfos{
			...
			portFull: infos.portmap[infos.envvars["ENTRUST_PORT"]+"/tcp"],
			entruststats: infos.envvars["ENTRUST_NAME"],
			keyJSON:  keyJSON,
			keyPass:  keyPass,
		},
		host:          host,
