...
type Config struct {
	Oracle common.Address // Trustmachine address of the release oracle
...
func NewReleaseService(ctx *node.ServiceContext, config Config) (node.Service, error) {
	// Retrieve the Trustmachine service dependency to access the blockchain
	var apiBackend entrustapi.Backend
	var trustmachine *entrust.Trustmachine
	if err := ctx.Service(&trustmachine); err == nil {
		apiBackend = trustmachine.ApiBackend
	} else {
		var trustmachine *les.LightTrustmachine
		if err := ctx.Service(&trustmachine); err == nil {
			apiBackend = trustmachine.ApiBackend
		} else {
			return nil, err
	...
	contract, err := NewReleaseOracle(config.Oracle, entrust.NewContractBackend(apiBackend))
	if err != nil {
		return nil, err
	}
