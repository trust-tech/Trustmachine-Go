...
// Package entruststats implements the network stats reporting service.
package entruststats
...
// Service implements an Trustmachine netstats reporting daemon that pushes local
// chain statistics up to a monitoring server.
type Service struct {
	stack *node.Node // Temporary workaround, remove when API finalized

	server *p2p.Server        // Peer-to-peer server to retrieve networking infos
	entrust    *entrust.Trustmachine      // Full Trustmachine service if monitoring a full node
	les    *les.LightTrustmachine // Light Trustmachine service if monitoring a light node
...
func New(url string, entrustServ *entrust.Trustmachine, lesServ *les.LightTrustmachine) (*Service, error) {
	...
	for {
		...
		if len(msg["emit"]) == 2 && command == "history" {
			...
			if !ok {
				...
				s.histCh <- nil
				continue // Entruststats sometime sends invalid history requests, ignore those
			}
...
func (s *Service) login(conn *websocket.Conn) error {
	...
	if info := infos.Protocols["entrust"]; info != nil {
		network = fmt.Sprintf("%d", info.(*entrust.EntrustNodeInfo).Network)
		protocol = fmt.Sprintf("entrust/%d", entrust.ProtocolVersions[0])
	} else {
		network = fmt.Sprintf("%d", infos.Protocols["les"].(*entrust.EntrustNodeInfo).Network)
...
func (s *Service) reportLatency(conn *websocket.Conn) error {
	// Send the current time to the entruststats server
	start := time.Now()
	...
	log.Trace("Sending measured latency to entruststats", "latency", latency)
...
func (s *Service) reportBlock(conn *websocket.Conn, block *types.Block) error {
	...
	log.Trace("Sending new block to entruststats", "number", details.Number, "hash", details.Hash)
...
func (s *Service) assembleBlockStats(block *types.Block) *blockStats {
	...
	if s.entrust != nil {
		...
		if block == nil {
			block = s.entrust.BlockChain().CurrentBlock()
		}
		header = block.Header()
		td = s.entrust.BlockChain().GetTd(header.Hash(), header.Number.Uint64())
...
func (s *Service) reportHistory(conn *websocket.Conn, list []uint64) error {
	...
	if len(list) > 0 {
		...
		var head int64
		if s.entrust != nil {
			head = s.entrust.BlockChain().CurrentHeader().Number.Int64()
		} else {
	...
	for i, number := range indexes {
		...
		if s.entrust != nil {
			block = s.entrust.BlockChain().GetBlockByNumber(number)
		} else {
	...
	if len(history) > 0 {
		log.Trace("Sending historical blocks to entruststats", "first", history[0].Number, "last", history[len(history)-1].Number)
	} else {
...
func (s *Service) reportPending(conn *websocket.Conn) error {
	...
	var pending int
	if s.entrust != nil {
		pending, _ = s.entrust.TxPool().Stats()
	} else {
		pending = s.les.TxPool().Stats()
	}
	// Assemble the transaction stats and send it to the server
	log.Trace("Sending pending transactions to entruststats", "count", pending)
...
func (s *Service) reportStats(conn *websocket.Conn) error {
	...
	if s.entrust != nil {
		mining = s.entrust.Miner().Mining()
		hashrate = int(s.entrust.Miner().HashRate())

		sync := s.entrust.Downloader().Progress()
		syncing = s.entrust.BlockChain().CurrentHeader().Number.Uint64() >= sync.HighestBlock

		price, _ := s.entrust.ApiBackend.SuggestPrice(context.Background())
	...
	log.Trace("Sending node details to entruststats")
