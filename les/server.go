...
func NewLesServer(entrust *entrust.Trustmachine, config *entrust.Config) (*LesServer, error) {
	...
	srv.fcCostStats = newCostStats(entrust.ChainDb())
	return srv, nil
}
...
func newCostStats(db entrustdb.Database) *requestCostStats {
...
func getChtRoot(db entrustdb.Database, num uint64) common.Hash {
...
func storeChtRoot(db entrustdb.Database, num uint64, root common.Hash) {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], num)
	db.Put(append(chtPrefix, encNumber[:]...), root[:])
}

func makeCht(db entrustdb.Database) bool {
