...
const (
	...
	entrustVersion = 63 // equivalent entrust version for the downloader
...
func NewProtocolManager(chainConfig *params.ChainConfig, lightSync bool, networkId uint64, mux *event.TypeMux, engine consensus.Engine, blockchain BlockChain, txpool txPool, chainDb entrustdb.Database, odr *LesOdr, txrelay *LesTxRelay) (*ProtocolManager, error) {
...
func (pm *ProtocolManager) removePeer(id string) {
	...
	log.Debug("Removing light Trustmachine peer", "peer", id)
...
func (pm *ProtocolManager) Stop() {
	...
	log.Info("Stopping light Trustmachine protocol")
	...
	log.Info("Light Trustmachine protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("Light Trustmachine peer registration failed", "err", err)
		return err
	}
	defer func() {
	...
	if pm.lightSync {
		...
		if err := pm.downloader.RegisterPeer(p.id, entrustVersion, p.HeadAndTd,
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Light Trustmachine message handling failed", "err", err)
			return err
		}
	}
}
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	p.Log().Trace("Light Trustmachine message arrived", "code", msg.Code, "bytes", msg.Size)
...
func (self *ProtocolManager) NodeInfo() *entrust.EntrustNodeInfo {
	return &entrust.EntrustNodeInfo{
