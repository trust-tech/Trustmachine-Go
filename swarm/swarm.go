...
import (
	...
	"github.com/ThePleasurable/go-trustmachine/contracts/ens"
	...
	"github.com/ThePleasurable/go-trustmachine/swarm/fuse"
...
func NewSwarm(ctx *node.ServiceContext, backend chequebook.Backend, config *api.Config, swapEnabled, syncEnabled bool, cors string) (self *Swarm, err error) {
	...
	if backend == (*entrustclient.Client)(nil) {
		log.Warn("No ENS, please specify non-empty --entrustapi to use domain name resolution")
	} else {
