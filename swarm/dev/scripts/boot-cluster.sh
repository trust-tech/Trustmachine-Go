...
#
# The cluster contains a bootnode, a gotrust node and multiple swarm nodes, with
...
# static gotrust configuration
GOTRUST_IP="192.168.33.3"
GOTRUST_RPC_PORT="8545"
GOTRUST_RPC_URL="http://${GOTRUST_IP}:${GOTRUST_RPC_PORT}"
...
# network namespaces togotruster
create_network() {
...
# start_bootnode starts a bootnode which is used to bootstrap the gotrust and
# swarm nodes
start_bootnode() {
...
# start_gotrust_node starts a gotrust node with --datadir pointing at <base-dir>/gotrust
# and a single, unlocked account with password "gotrust"
start_gotrust_node() {
  local dir="${base_dir}/gotrust"
  mkdir -p "${dir}"

  local password="gotrust"
  echo "${password}" > "${dir}/password"

  # create an account if necessary
  if [[ ! -e "${dir}/keystore" ]]; then
    info "creating gotrust account"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get gotrust account address"
  fi
  ...
  local args=(
    ...
    --rpc
    --rpcaddr   "${GOTRUST_IP}"
    --rpcport   "${GOTRUST_RPC_PORT}"
    --verbosity "6"
  )

  start_node "gotrust" "${GOTRUST_IP}" "$(which gotrust)" ${args[@]}
}
...
create_node_network() {
  ...
  # create the ventrust pair
  local ventrust0="ventrust${name}0"
  local ventrust1="ventrust${name}1"
  ip link add name "${ventrust0}" type ventrust peer name "${ventrust1}"

  # add one end to the bridge
  ip link set dev "${ventrust0}" master "${BRIDGE_NAME}"
  ip link set dev "${ventrust0}" up

  # add the other end to the namespace, rename it entrust0 and give it the ip
  ip link set dev "${ventrust1}" netns "${name}"
  ip netns exec "${name}" ip link set dev "${ventrust1}" name "entrust0"
  ip netns exec "${name}" ip link set dev "entrust0" up
  ip netns exec "${name}" ip address add "${ip}/24" dev "entrust0"
}
...
create_account() {
  ...
  gotrust --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
