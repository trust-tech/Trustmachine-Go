...
import (
	...
	"github.com/ThePleasurable/go-trustmachine/logger"
	"github.com/ThePleasurable/go-trustmachine/p2p"
	"github.com/ThePleasurable/go-trustmachine/p2p/nat"
	"github.com/ThePleasurable/go-trustmachine/whisper"
)
...
func main() {
	...
	// Create an Trustmachine peer to communicate through
	server := p2p.Server{
		PrivateKey: key,
		MaxPeers:   10,
		Name:       name,
	...
	fmt.Println("Starting Trustmachine peer...")
	if err := server.Start(); err != nil {
		fmt.Printf("Failed to start Trustmachine peer: %v.\n", err)
		os.Exit(1)
	}

	// Send a message to self to check that somentrusting works
