...
// header only chain.
func newCanonical(n int) (entrustdb.Database, *LightChain, error) {
	db, _ := entrustdb.NewMemDatabase()
	gspec := core.Genesis{Config: params.TestChainConfig}
	genesis := gspec.MustCommit(db)
	blockchain, _ := NewLightChain(&dummyOdr{db: db}, gspec.Config, entrustash.NewFaker(), new(event.TypeMux))
...
func newTestLightChain() *LightChain {
	...
	gspec.MustCommit(db)
	lc, err := NewLightChain(&dummyOdr{db: db}, gspec.Config, entrustash.NewFullFaker(), new(event.TypeMux))
	if err != nil {
		panic(err)
	}
	return lc
...
type dummyOdr struct {
	OdrBackend
	db entrustdb.Database
}

func (odr *dummyOdr) Database() entrustdb.Database {
	return odr.db
}
...
func TestReorgBadHeaderHashes(t *testing.T) {
	...
	ncm, err := NewLightChain(&dummyOdr{db: bc.chainDb}, params.TestChainConfig, entrustash.NewFaker(), new(event.TypeMux))
	if err != nil {
