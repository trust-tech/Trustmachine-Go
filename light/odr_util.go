...
func GetTrustedCht(db entrustdb.Database) TrustedCht {
...
func WriteTrustedCht(db entrustdb.Database, cht TrustedCht) {
	data, _ := rlp.EncodeToBytes(cht)
	db.Put(trustedChtKey, data)
}

func DeleteTrustedCht(db entrustdb.Database) {
