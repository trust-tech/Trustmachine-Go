...
import (
	...
	"github.com/ThePleasurable/go-trustmachine/accounts/usbwallet"
...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of gotrust is "gotrust". If no
...
func (c *Config) NodeName() string {
	name := c.name()
	// Backwards compatibility: previous versions used title-cased "Gotrust", keep that.
	if name == "gotrust" || name == "gotrust-testnet" {
		name = "Gotrust"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "gotrust" instances.
var isOldGotrustResource = map[string]bool{
	"chaindata":          true,
	"nodes":              true,
	"nodekey":            true,
...
func (c *Config) resolvePath(path string) string {
	...
	// by gotrust 1.4 are used if they exist.
	if c.name() == "gotrust" && isOldGotrustResource[path] {
		oldpath := ""
		if c.Name == "gotrust" {
...
func makeAccountManager(conf *Config) (*accounts.Manager, string, error) {
	...
	default:
		// There is no datadir.
		keydir, err = ioutil.TempDir("", "go-trustmachine-keystore")
		ephemeral = keydir
	}
	if err != nil {
		return nil, "", err
