...
// Entrustash proof-of-work protocol constants.
var (
...
func (entrustash *Entrustash) VerifyHeader(chain consensus.ChainReader, header *types.Header, seal bool) error {
	...
	return entrustash.verifyHeader(chain, header, parent, false, seal)
}
...
func (entrustash *Entrustash) VerifyHeaders(chain consensus.ChainReader, headers []*types.Header, seals []bool) (chan<- struct{}, <-chan error) {
	// If we're running a full engine faking, accept any input as valid
	if entrustash.fakeFull || len(headers) == 0 {
	...
	for i := 0; i < workers; i++ {
		go func() {
			for index := range inputs {
				errors[index] = entrustash.verifyHeaderWorker(chain, headers, seals, index)
				done <- index
			}
		}()
	}
...
func (entrustash *Entrustash) verifyHeaderWorker(chain consensus.ChainReader, headers []*types.Header, seals []bool, index int) error {
	...
	return entrustash.verifyHeader(chain, headers[index], parent, false, seals[index])
}

// VerifyUncles verifies that the given block's uncles conform to the consensus
// rules of the stock Trustmachine entrustash engine.
func (entrustash *Entrustash) VerifyUncles(chain consensus.ChainReader, block *types.Block) error {
	...
	for _, uncle := range block.Uncles() {
		...
		if err := entrustash.verifyHeader(chain, uncle, ancestors[uncle.ParentHash], true, true); err != nil {
			return err
		}
	}
	return nil
...
func (entrustash *Entrustash) verifyHeader(chain consensus.ChainReader, header, parent *types.Header, uncle bool, seal bool) error {
	...
	if seal {
		if err := entrustash.VerifySeal(chain, header); err != nil {
			return err
		}
	}
...
func calcDifficultyMetropolis(time uint64, parent *types.Header) *big.Int {
	// https://github.com/trust-tech/EIPs/issues/100.
	// algorithm:
	// diff = (parent_diff +
...
func calcDifficultyHomestead(time uint64, parent *types.Header) *big.Int {
	// https://github.com/trust-tech/EIPs/blob/master/EIPS/eip-2.mediawiki
	// algorithm:
	// diff = (parent_diff +
...
func (entrustash *Entrustash) VerifySeal(chain consensus.ChainReader, header *types.Header) error {
	...
	if entrustash.fakeMode {
		time.Sleep(entrustash.fakeDelay)
		if entrustash.fakeFail == header.Number.Uint64() {
	...
	if entrustash.shared != nil {
		return entrustash.shared.VerifySeal(chain, header)
	}
	...
	cache := entrustash.cache(number)

	size := datasetSize(number)
	if entrustash.tester {
		size = 32 * 1024
	}
...
// header to conform to the entrustash protocol. The changes are done inline.
