Name "gotrust ${MAJORVERSION}.${MINORVERSION}.${BUILDVERSION}" # VERSION variables set through command line arguments
InstallDir "$InstDir"
...
# Install gotrust binary
Section "Gotrust" GOTRUST_IDX
  SetOutPath $INSTDIR
  file {{.Gotrust}}

  # Create start menu launcher
  createDirectory "$SMPROGRAMS\${APPNAME}"
  createShortCut "$SMPROGRAMS\${APPNAME}\${APPNAME}.lnk" "$INSTDIR\gotrust.exe" "--fast" "--cache=512"
  createShortCut "$SMPROGRAMS\${APPNAME}\Attach.lnk" "$INSTDIR\gotrust.exe" "attach" "" ""
  ...
  # Firewall - add rules
  SimpleFC::AdvAddRule "Gotrust incoming peers (TCP:30303)" ""  6 1 1 2147483647 1 "$INSTDIR\gotrust.exe" "" "" "Trustmachine" 30303 "" "" ""
...
Function GetInstalledSize
  ...
  ${if} ${SectionIsSelected} ${GOTRUST_IDX}
    SectionGetSize ${GOTRUST_IDX} $0
