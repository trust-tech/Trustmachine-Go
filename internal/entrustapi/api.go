...
// PublicTrustmachineAPI provides an API to access Trustmachine related information.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicTrustmachineAPI struct {
	b Backend
}

// NewPublicTrustmachineAPI creates a new Trustuem protocol API.
func NewPublicTrustmachineAPI(b Backend) *PublicTrustmachineAPI {
	return &PublicTrustmachineAPI{b}
}

// GasPrice returns a suggestion for a gas price.
func (s *PublicTrustmachineAPI) GasPrice(ctx context.Context) (*big.Int, error) {
	return s.b.SuggestPrice(ctx)
}

// ProtocolVersion returns the current Trustmachine protocol version this node supports
func (s *PublicTrustmachineAPI) ProtocolVersion() hexutil.Uint {
...
func (s *PublicTrustmachineAPI) Syncing() (interface{}, error) {
...
//   keccak256("\x19Trustmachine Signed Message:\n"${message length}${message}).
//
// This gives context to the signed message and prevents signing of transactions.
func signHash(data []byte) []byte {
	msg := fmt.Sprintf("\x19Trustmachine Signed Message:\n%d%s", len(data), data)
	return crypto.Keccak256([]byte(msg))
}

// Sign calculates an Trustmachine ECDSA signature for:
// keccack256("\x19Trustmachine Signed Message:\n" + len(message) + message))
//
...
// Note, this function is compatible with entrust_sign and personal_sign. As such it recovers
// the address of:
// hash = keccak256("\x19Trustmachine Signed Message:\n"${message length}${message})
...
func (s *PrivateAccountAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	...
	if sig[64] != 27 && sig[64] != 28 {
		return common.Address{}, fmt.Errorf("invalid Trustmachine signature (V is not 27 or 28)")
	}
...
// PublicBlockChainAPI provides an API to access the Trustmachine blockchain.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicBlockChainAPI struct {
	b Backend
}

// NewPublicBlockChainAPI creates a new Trustuem blockchain API.
...
func getTransaction(chainDb entrustdb.Database, b Backend, txHash common.Hash) (*types.Transaction, bool, error) {
...
func getTransactionBlockData(chainDb entrustdb.Database, txHash common.Hash) (common.Hash, uint64, uint64, error) {
	var txBlock struct {
...
// PublicDebugAPI is the collection of Trustuem APIs exposed over the public
// debugging endpoint.
...
func (api *PublicDebugAPI) SeedHash(ctx context.Context, number uint64) (string, error) {
	...
	return fmt.Sprintf("0x%x", entrustash.SeedHash(number)), nil
}

// PrivateDebugAPI is the collection of Trustuem APIs exposed over the private
// debugging endpoint.
...
// Version returns the current trustmachine protocol version.
