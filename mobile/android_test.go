...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", Gotrust.LightScryptN, Gotrust.LightScryptP);
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.trustmachine", new NodeConfig());
			node.start();
			...
			TrustmachineClient ec = node.getTrustmachineClient();
	...
	// signatures too. Regression test for go-trustmachine issue #14599.
...
func TestAndroid(t *testing.T) {
	// Skip tests on Windows altogotruster
	...
	workspace, err := ioutil.TempDir("", "gotrust-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/trustmachine/gotrusttest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for Gotrust and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.trustmachine", "github.com/trust-tech/go-trustmachine/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	build.CopyFile(filepath.Join("libs", "gotrust.aar"), "gotrust.aar", os.ModePerm)

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "trustmachine", "gotrusttest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
...
<manifest xmlns:android="http://schemas.android.com/apk/res/android"
          package="org.trustmachine.gotrusttest"
