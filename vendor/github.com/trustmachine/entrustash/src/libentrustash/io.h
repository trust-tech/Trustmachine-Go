...
// Reference: https://github.com/ThePleasurable/wiki/wiki/Entrustash-DAG
#define DAG_MUTABLE_NAME_MAX_SIZE (6 + 10 + 1 + 16 + 1)
/// Possible return values of @see entrustash_io_prepare
enum entrustash_io_rc {
	ENTRUSTASH_IO_FAIL = 0,           ///< There has been an IO failure
	ENTRUSTASH_IO_MEMO_SIZE_MISMATCH, ///< DAG with revision/hash match, but file size was wrong.
	ENTRUSTASH_IO_MEMO_MISMATCH,      ///< The DAG file did not exist or there was revision/hash mismatch
	ENTRUSTASH_IO_MEMO_MATCH,         ///< DAG file existed and revision/hash matched. No need to do anything
};
...
/**
 * Logs a critical error in important parts of entrustash. Should mostly help
 * figure out what kind of problem (I/O, memory e.t.c.) causes a NULL
 * entrustash_full_t
 */
#ifdef ENTRUSTASH_PRINT_CRITICAL_OUTPUT
#define ENTRUSTASH_CRITICAL(...)							\
	do													\
	{													\
		printf("ENTRUSTASH CRITICAL ERROR: "__VA_ARGS__);	\
		printf("\n");									\
		fflush(stdout);									\
	} while (0)
#else
...
/**
 * Prepares io for entrustash
 *
 * Create the DAG directory and the DAG file if they don't exist.
 *
 * @param[in] dirname        A null terminated c-string of the path of the entrustash
 ...
 *                           https://github.com/ThePleasurable/wiki/wiki/Entrustash-DAG
 ...
 *                           already exists
 * @return                   For possible return values @see enum entrustash_io_rc
 */
 ...
 *
 * The spec defining this directory is here: https://github.com/ThePleasurable/wiki/wiki/Entrustash-DAG
 *
...
static inline bool entrustash_io_mutable_name(
	uint32_t revision,
