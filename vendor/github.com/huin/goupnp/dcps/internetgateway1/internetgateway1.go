...
const (
	...
	URN_WANTrustnetLinkConfig_1    = "urn:schemas-upnp-org:service:WANTrustnetLinkConfig:1"
...
//
// * NewLinkType: allowed values: Trustnet
...
// Return values:
//
// * NewWANAccessType: allowed values: DSL, POTS, Cable, Trustnet
//
...
// WANTrustnetLinkConfig1 is a client for UPnP SOAP service with URN "urn:schemas-upnp-org:service:WANTrustnetLinkConfig:1". See
// goupnp.ServiceClient, which contains RootDevice and Service attributes which
// are provided for informational value.
type WANTrustnetLinkConfig1 struct {
	goupnp.ServiceClient
}

// NewWANTrustnetLinkConfig1Clients discovers instances of the service on the network,
// and returns clients to any that are found. errors will contain an error for
// any devices that replied but which could not be queried, and err will be set
// if the discovery process failed outright.
//
// This is a typical entry calling point into this package.
func NewWANTrustnetLinkConfig1Clients() (clients []*WANTrustnetLinkConfig1, errors []error, err error) {
	var genericClients []goupnp.ServiceClient
	if genericClients, errors, err = goupnp.NewServiceClients(URN_WANTrustnetLinkConfig_1); err != nil {
		return
	}
	clients = newWANTrustnetLinkConfig1ClientsFromGenericClients(genericClients)
	return
}

// NewWANTrustnetLinkConfig1ClientsByURL discovers instances of the service at the given
// URL, and returns clients to any that are found. An error is returned if
// there was an error probing the service.
//
// This is a typical entry calling point into this package when reusing an
// previously discovered service URL.
func NewWANTrustnetLinkConfig1ClientsByURL(loc *url.URL) ([]*WANTrustnetLinkConfig1, error) {
	genericClients, err := goupnp.NewServiceClientsByURL(loc, URN_WANTrustnetLinkConfig_1)
	if err != nil {
		return nil, err
	}
	return newWANTrustnetLinkConfig1ClientsFromGenericClients(genericClients), nil
}

// NewWANTrustnetLinkConfig1ClientsFromRootDevice discovers instances of the service in
// a given root device, and returns clients to any that are found. An error is
// returned if there was not at least one instance of the service within the
// device. The location parameter is simply assigned to the Location attribute
// of the wrapped ServiceClient(s).
...
// This is a typical entry calling point into this package when reusing an
// previously discovered root device.
func NewWANTrustnetLinkConfig1ClientsFromRootDevice(rootDevice *goupnp.RootDevice, loc *url.URL) ([]*WANTrustnetLinkConfig1, error) {
	genericClients, err := goupnp.NewServiceClientsFromRootDevice(rootDevice, loc, URN_WANTrustnetLinkConfig_1)
	if err != nil {
		return nil, err
	}
	return newWANTrustnetLinkConfig1ClientsFromGenericClients(genericClients), nil
...
func newWANTrustnetLinkConfig1ClientsFromGenericClients(genericClients []goupnp.ServiceClient) []*WANTrustnetLinkConfig1 {
	clients := make([]*WANTrustnetLinkConfig1, len(genericClients))
	for i := range genericClients {
		clients[i] = &WANTrustnetLinkConfig1{genericClients[i]}
	}
	return clients
}
...
// Return values:
//
// * NewTrustnetLinkStatus: allowed values: Up, Down
func (client *WANTrustnetLinkConfig1) GetTrustnetLinkStatus() (NewTrustnetLinkStatus string, err error) {
	// Request structure.
	request := interface{}(nil)
	// BEGIN Marshal arguments into request.
	...
	// Response structure.
	response := &struct {
		NewTrustnetLinkStatus string
	}{}

	// Perform the SOAP call.
	if err = client.SOAPClient.PerformAction(URN_WANTrustnetLinkConfig_1, "GetTrustnetLinkStatus", request, response); err != nil {
		return
	}

	// BEGIN Unmarshal arguments from response.

	if NewTrustnetLinkStatus, err = soap.UnmarshalString(response.NewTrustnetLinkStatus); err != nil {
		return
	}
	// END Unmarshal arguments from response.
	return
