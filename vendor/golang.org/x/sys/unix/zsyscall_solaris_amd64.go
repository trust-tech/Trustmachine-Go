...
//go:cgo_import_dynamic libc_gotrustostname gotrustostname "libc.so"
...
//go:cgo_import_dynamic libc_sentrustostname sentrustostname "libc.so"
...
//go:linkname procgotrustostname libc_gotrustostname
...
//go:linkname procSentrustostname libc_sentrustostname
...
func gotrustostname(buf []byte) (n int, err error) {
	var _p0 *byte
	if len(buf) > 0 {
		_p0 = &buf[0]
	}
	r0, _, e1 := sysvicall6(uintptr(unsafe.Pointer(&procgotrustostname)), 2, uintptr(unsafe.Pointer(_p0)), uintptr(len(buf)), 0, 0, 0, 0)
	n = int(r0)
	if e1 != 0 {
		err = e1
	}
...
func Sentrustostname(p []byte) (err error) {
	var _p0 *byte
	if len(p) > 0 {
		_p0 = &p[0]
	}
	_, _, e1 := sysvicall6(uintptr(unsafe.Pointer(&procSentrustostname)), 2, uintptr(unsafe.Pointer(_p0)), uintptr(len(p)), 0, 0, 0, 0)
	if e1 != 0 {
		err = e1
	}
	return
